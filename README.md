# 🐋 dsh-whale — DeepSeek Harness 的小鲸鱼伴侣插件

住在 DeepSeek Harness（DSH）Web 界面右下角的打工小鲸鱼：任务开工/完成/失败/提问播报、
每轮 tokens 用量面板、长任务计时、跨对话后台通知、历史任务（搜索/范围清空）与周报、
定时提醒（倒计时或指定到时分秒）、会话跳转、操作手册、装扮养成。
**只读事件流，不拦截、不修改任何任务行为。**

> **定位一句话**：本仓库的核心交付物是 **DSH 的 Web UI 插件**（`src/` + `whale-assistant/`）；
> `shell/` 里的桌面壳是**可选附赠品**——插件只要求 DSH web 在跑，用浏览器直开、
